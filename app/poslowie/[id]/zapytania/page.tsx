...
import { fetchMPs, fetchWrittenQuestions } from '@/lib/api'
import { notFound } from 'next/navigation'
import { WrittenQuestions } from '@/components/written-questions'
import { Loader2, AlertCircle, RefreshCw, ArrowLeft } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { useParams } from 'next/navigation'
import Link from 'next/link'

export default function MPWrittenQuestionsPage() {
  ...
  const { data: writtenQuestions, isLoading, error, refetch, isRefetching } = useQuery({
    queryKey: ['written-questions', mpId],
    queryFn: () => fetchWrittenQuestions(mpId),
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <h1 className="text-3xl font-bold text-foreground">
            Zapytania poselskie - {mp.firstLastName}
          </h1>
          <p className="text-muted-foreground">
            Lista wszystkich zapytań poselskich złożonych przez {mp.firstLastName} w X kadencji Sejmu RP
          </p>
        </div>
        ...
        <div className="flex items-center justify-center min-h-[40vh]">
          <div className="text-center space-y-4">
            ...
            <div className="space-y-2">
              <p className="text-lg font-medium text-foreground">Ładowanie zapytań poselskich...</p>
      ...
      <div className="space-y-6">
        ...
        <div className="text-center py-16">
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="text-destructive text-xl font-semibold">
                Błąd podczas ładowania zapytań
              </div>
              <div className="text-sm text-muted-foreground max-w-md mx-auto">
                Nie udało się pobrać zapytań poselskich z API Sejmu RP. Sprawdź połączenie internetowe i spróbuj ponownie.
              </div>
              {error && (
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-4 text-sm text-muted-foreground">
          ...
          <span>•</span>
          <span>Zapytań: <strong className="text-foreground">{writtenQuestions?.length || 0}</strong></span>
        </div>
      </div>
